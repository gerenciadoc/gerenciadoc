...
// Implementação da interface de usuário integrada com o backend
...
async function loadDocuments() {
    try {
        ...
        const response = await api.getDocuments(currentFilters);
...
function populateCategoryFilters(categories) {
    ...
    categories.forEach(category => {
        ...
        if (uploadCategorySelect) {
            uploadCategorySelect.appendChild(option.cloneNode(true));
        }
...
function populateDocumentTypeSelects(types) {
    ...
    const editTypeSelect = document.getElementById('editDocumentType');
    ...
    if (uploadTypeSelect) {
        ...
        types.forEach(type => {
            ...
            uploadTypeSelect.appendChild(option.cloneNode(true));
        });
    }
    
    if (editTypeSelect) {
        // Limpar opções existentes
        editTypeSelect.innerHTML = '<option value="">Selecione um tipo</option>';
...
async function extractDocumentData(file) {
    try {
        ...
        const response = await api.extractDocumentData(formData);
        
        // Preencher formulário com dados extraídos
        if (response.success) {
            populateFormWithExtractedData(response.extractedData);
...
async function uploadDocument(e) {
    ...
    try {
        ...
        // Processar tags
        const tagsInput = form.querySelector('#documentTags');
        ...
        // Processar metadados
        const metadataFields = form.querySelectorAll('[name^="metadata."]');
...
async function viewDocument(id) {
    try {
        ...
        if (response.success) {
            ...
            const downloadButton = modal.querySelector('.modal-footer .btn-primary');
            downloadButton.onclick = () => downloadDocument(document._id, document.fileUrl);
...
async function saveDocumentEdit(e) {
    ...
    try {
        ...
        // Enviar para API
        const response = await api.updateDocument(documentId, documentData);
